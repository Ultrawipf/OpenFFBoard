...
void Axis::registerCommands(){
	...
	registerCommand("reconfilter", Axis_commands::reconfilter, "Reconstruction filter mode (0=None, 1=Responsive, 2=Quick, 3=Mixed)", CMDFLAG_GET | CMDFLAG_SET);
	...
	registerCommand("maxdrvslewrate", Axis_commands::maxdrvslewrate, "Max driver torque rate limit in counts/ms",CMDFLAG_GET);
}
...
void Axis::restoreFlash(){
	...
	if(Flash_Read(flashAddresses.postprocess1, &pp1)){
		...
		reconFilterMode = (ReconFilterMode)((pp1 >> 8) & 0x03);
		equalizerEnabled = (pp1 >> 10) & 0x01;
	}
...
void Axis::saveFlash(){
	...
	// Postprocessing
	uint16_t pp1 = (expoValue & 0xff) | ((uint8_t)reconFilterMode << 8) | (equalizerEnabled << 10);
...
void Axis::setDrvType(uint8_t drvtype)
{
	...
	this->maxTorqueRateMS = drv->getMaxSlewRate();
...
void Axis::resetMetrics(float new_pos= 0) { // pos is degrees
	...
	arm_pid_reset_f32(&speedLimiterPID);
	last_interpolated_torque = 0;
}
...
int64_t Axis::calculateFFBTorque(){
	...
	int64_t torque_from_game = getInterpolatedTorque();

	// Apply equalizer
	float filtered_torque = torque_from_game;
	if(equalizerEnabled){
	...
	// Game effects. Down and upscaling may introduce float artifacts. Do this before scaling down.
...
int64_t Axis::getInterpolatedTorque(){

	int64_t resulting_torque = 0;

	switch(reconFilterMode){

		// If no reconstruction filter is enable we get directly the last Torque
		case ReconFilterMode::NO_RECONSTRUCTION:
			resulting_torque = torque_t1;
			break;

		// Mode 0: Linear Interpolation with Slew Rate Clipping (Responsive)
		// This mode calculates the ideal torque based on linear interpolation between the last two game packets.
		// It then applies a slew rate limiter to this ideal value.
		// Pro: More responsive to the game's intended force curve. Tries to "catch up" to the ideal line.
		// Con: Can feel less smooth if the slew rate limit is hit frequently, as the slope of the torque can change abruptly.
		case ReconFilterMode::LINEAR_INTERPOLATION:
		case ReconFilterMode::LINEAR_AND_SLEWRATE:
		{
			uint32_t now_us = micros();
			uint32_t interval_duration = time_t1_us - time_t0_us;
			int64_t interpolated_torque = torque_t1; // Default to the latest torque value

			// Only interpolate if the time interval is valid and we are not too far past the last update (e.g., game paused).
			if (interval_duration > 0 && (now_us - time_t1_us) < 50000) { // 50ms timeout
				float progress = (float)(now_us - time_t1_us) / (float)interval_duration;
				progress = clip<float>(progress, 0.0f, 1.0f); // Clamp progress to prevent extrapolation.
				interpolated_torque = torque_t0 + progress * (torque_t1 - torque_t0);
			}

			if ((reconFilterMode == ReconFilterMode::LINEAR_AND_SLEWRATE) && (maxTorqueRateMS > 0)) {
				const int64_t max_torque_change = maxTorqueRateMS; // Since this is called every 1ms
				resulting_torque = clip<int64_t>(interpolated_torque, last_interpolated_torque - max_torque_change, last_interpolated_torque + max_torque_change);
			} else {
				resulting_torque = interpolated_torque;
			}
			break;
		}

		// Mode 1: Slew Rate Limited Target Following (Smooth)
		// This mode treats the last torque value from the game as a target.
		// It moves the current torque towards this target at a rate no faster than maxTorqueRateMS.
		// Pro: Guarantees a smooth torque delivery that always respects the motor's physical limits.
		// Con: May feel less detailed or "rounded off" on very sharp/fast force effects, as it prioritizes smoothness over responsiveness.
		case ReconFilterMode::SLEW_RATE_LIMITED:
		{
			if (maxTorqueRateMS == 0) {
				resulting_torque = torque_t1;
				break;
			}
			const int64_t target_torque = torque_t1;
			int64_t current_torque = last_interpolated_torque;
			int64_t delta = target_torque - current_torque;
			const int64_t max_change = maxTorqueRateMS;

			if (abs(delta) > max_change) {
				current_torque += (delta > 0) ? max_change : -max_change;
			} else {
				current_torque = target_torque;
			}
			resulting_torque = current_torque;
			break;
		}
	}

	last_interpolated_torque = resulting_torque; // Store the final value for the next iteration.
...
void Axis::setFfbEffectTorque(int64_t torque) {
	// This function is called by the EffectsCalculator when a new FFB torque value from the game is ready.
	// It updates the start (t0) and end (t1) points for the linear interpolation.
	torque_t0 = torque_t1;
	time_t0_us = time_t1_us;
...
CommandStatus Axis::command(const ParsedCommand& cmd,std::vector<CommandReply>& replies){
	...
	case Axis_commands::slewrate:
		{
			uint16_t drvSlewRate = drv->getMaxSlewRate();
			if(cmd.type == CMDtype::get){
				// If driver has a more restrictive calibrated value, update the axis limit
				if(drvSlewRate > 0 && drvSlewRate < this->maxTorqueRateMS)
					this->maxTorqueRateMS = drvSlewRate;
				replies.emplace_back(this->maxTorqueRateMS);
			}else if(cmd.type == CMDtype::set){
				this->maxTorqueRateMS = clip<uint32_t,uint32_t>(cmd.val, 0, drvSlewRate);
			}
		}
		break;

	case Axis_commands::maxdrvslewrate:
		if (cmd.type == CMDtype::get) {
			replies.emplace_back(drv->getMaxSlewRate());
		}
		break;
...
//#endif
	...
	case Axis_commands::reconfilter:
		if(cmd.type == CMDtype::get){
			replies.emplace_back((uint8_t)this->reconFilterMode);
		}else if(cmd.type == CMDtype::set){
			this->reconFilterMode = (ReconFilterMode)clip<uint8_t,uint8_t>(cmd.val, 0, 3);
		}
		break;
