...
enum class ReconFilterMode : uint8_t {
    ...
    SLEW_RATE_LIMITED = 2,    	// Smoother, follows the game's signal within the motor's physical limits. Might round off sharp force peaks.
	LINEAR_AND_SLEWRATE = 3		// Linear responsible, and the slew_rate_clipping
};
...
enum class Axis_commands : uint32_t{
	...
	maxspeed,slewrate,maxdrvslewrate,
	expo,exposcale,
	reconfilter,
...
private:
	...
	/**
	 * @brief Calculates the interpolated torque for the reconstruction filter.
	 * @return The interpolated torque value.
	 */
	int64_t getInterpolatedTorque();
	/**
	 * @brief Decodes the axis configuration from a 16-bit integer.
	 * @param val The value from flash.
	 * @return The decoded AxisConfig struct.
	 */
	...
	// Interpolation variables for reconstruction filter
	volatile int64_t torque_t0 = 0; 	//!< Previous torque sample from the game (start point).
	volatile int64_t torque_t1 = 0; 	//!< Most recent torque sample from the game (end point).
	volatile uint32_t time_t0_us = 0;	//!< Timestamp (in µs) of the previous torque sample.
	volatile uint32_t time_t1_us = 0;	//!< Timestamp (in µs) of the most recent torque sample.
	int64_t last_interpolated_torque = 0; //!< Stores the last output of the reconstruction filter for slew rate limiting.
	...
	ReconFilterMode reconFilterMode = ReconFilterMode::NO_RECONSTRUCTION; //!< Reconstruction filter algorithm selection.
